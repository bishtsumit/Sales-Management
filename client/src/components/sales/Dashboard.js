...
class Dashboard extends Component {
  constructor() {
    ...
    this.RenderDashboardData = this.RenderDashboardData.bind(this);
    this.BarChart = this.BarChart.bind(this);
    this.donutChart = this.donutChart.bind(this);
    this.InitializeBarChartAndRender = this.InitializeBarChartAndRender.bind(
      this
    );
    this.InitializeDonutChartAndRender = this.InitializeDonutChartAndRender.bind(
      this
    );
  }
  ...
  RenderDashboardData() {
    axios
      .post("/api/user/GetDashboardData")
      .then((res) => {
        console.log(res.data);

        this.setState({
          devicedetail: res.data.devicedetail,
          ArrSite: res.data.ArrSite,
          ArrSiteCount: res.data.ArrSiteCount,
          websiteAvg: res.data.websiteAvg,
          error: "",
          DataGet: "success",
        });
      })
  ...
  donutChart() {
    ...
    function chart(selection) {
      selection.each(function () {
        ...
        svg = d3.select("#svgDonut");

        d3.select("#svgDonut g").remove();
        ...
        var DataLength = d3.sum(data, (d) => d.value);
  ...
  componentDidMount() {
    ...
    //calling dashboard data function every 10 minutes
    setInterval(() => {
  ...
  componentDidUpdate() {
    ...
    // setting the height of table //
    var thead = $("#tableRegionAccess thead").height();
    var tableheight = $("#tableRegionAccess").parent().height(); // Parent Div height is fixed

    $("#tableRegionAccess tbody").height(tableheight - thead);

    var thead = $("#tblSales thead").height();
    var tableheight = $("#tblSales").parent().height(); // Parent Div height is fixed

    $("#tblSales tbody").height(tableheight - thead);

    this.InitializeBarChartAndRender("DivBarChart");
    this.InitializeDonutChartAndRender("DivDonutChart");
  }

  InitializeBarChartAndRender(ContainerId) {
    ...
    console.log(this.state.ArrSiteCount);

    Bar.data(this.state.ArrSiteCount);
  }

  InitializeDonutChartAndRender(ContainerId) {
    ...
    donut.data(this.state.devicedetail);
  }

  render() {
    const { devicedetail, ArrSite, ArrSiteCount, websiteAvg } = this.state;
    ...
    var content = (
      <div className="row">
        <div style={{ width: "100%" }}>
          <div className="row" id="DashboardDiv">
            <div className="col-lg-4 col-md-6 col-sm-12 mt-1 adjust">
              <div
                className="row card boxShadow"
                id="DivBarChart"
                style={{ height: "300px", boxShadow: "none", width: "100%" }}
              >
                <svg width="0%" height="420px" id="chart"></svg>
              </div>
            </div>
            <div className="col-lg-4 col-md-6 col-sm-12 mt-1 adjust">
              <div
                ...
                style={{ height: "300px", boxShadow: "none", width: "100%" }}
              >
                <svg width="0%" height="420px" id="svgDonut"></svg>
              </div>
            </div>
            <div className="col-lg-4 col-md-12 col-sm-12 mt-1 adjust">
              <div
                className="row card boxShadow"
                id="DivMapAccessTable"
                style={{ height: "300px", boxShadow: "none", width: "100%" }}
              >
                <table className="table CustomTable" id="tableRegionAccess">
                  <thead>
                    <tr
                      style={{
                        color: "rgb(68, 173, 220)",
                        fontSize: "12px",
                      }}
                    >
                      <th>Site Name</th>
                  ...
                  <tbody
                    id="MapAccessTableBody"
                    style={{ fontSize: "13px", color: "#8D97A9" }}
                  >
                    {websiteAvg.map((dr, idx) => (
                      <tr key={idx}>
                        <td>{dr.sitename}</td>
                        <td>{dr.average}</td>
                      </tr>
          ...
          <div className="row mt-5">
            <div className="col-lg-12" style={{ width: "100%" }}>
              <div
                className="row  card boxShadow"
                id="DivRecentSalesTable"
                style={{ height: "300px", boxShadow: "none" }}
              >
                <table className="table CustomTable" id="tblSales">
                  <thead>
                    <tr
                      style={{
                        color: "rgb(68, 173, 220)",
                    ...
                    >
                      ...
                      <th style={{ maxWidth: "130px" }}>Client Address</th>
    ...
    return (
      <div className="Dashboard">
        <div className="container-fluid" style={{ maxWidth: "90%" }}>
          {this.state.error == "" && this.state.DataGet == ""
            ? this.state.msg
            : this.state.error == "" && this.state.DataGet == "success"
            ? content
            : this.state.error}
        </div>
      </div>
